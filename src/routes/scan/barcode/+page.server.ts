  ...
  saveProduct: async ({ request, locals }) => {
    ...
    */
    ...
    if (!shelf_id || !scale_index_raw) {
      return fail(400, {
        error: "Context missing: shelf_id or slot (scale_index) not provided.",
      });
    }
    ...
    if (isNaN(scale_index) || scale_index < 0) {
      return fail(400, {
        error: "Invalid scale_index. Must be a non-negative integer.",
      });
    }
    ...
    */
    ...
    if (catError) {
      console.error(
        "Schema or RLS Error in product_catalog:",
        catError.message,
      );
